...
export const MainJS = () => {
  const { getCourseProgress, isLessonCompleted, markLessonComplete } =
    useProgress();
  const courseProgress = getCourseProgress("javascript");
  const [openMenuId, setOpenMenuId] = useState<number | null>(null);

  const chapters = [
    { id: 1, title: "JavaScript Basics and Syntax" },
    { id: 2, title: "Variables and Data Types" },
    { id: 3, title: "Functions and Scope" },
    { id: 4, title: "Arrays and Objects" },
    { id: 5, title: "Control Flow and Conditionals" },
    { id: 6, title: "Loops and Iteration" },
    { id: 7, title: "DOM Manipulation Basics" },
    { id: 8, title: "Events and Event Handling" },
    { id: 9, title: "Dynamic Content and Interaction" },
    { id: 10, title: "JavaScript Best Practices" },
    { id: 11, title: "Modern JavaScript Features" },
    { id: 12, title: "Debugging and Error Handling" },
  ];
  ...
  const handleMarkComplete = async (e: React.MouseEvent, chapterId: number) => {
    e.stopPropagation();
    await markLessonComplete("javascript", chapterId);
    setOpenMenuId(null);
  };

  const handleOpenLesson = (chapterId: number) => {
    navigate(`/lessons/js/${chapterId}`);
    setOpenMenuId(null);
  };
        ...
        <div className="flex items-center mt-10 mb-8 border justify-evenly w-[92%] mx-auto p-6 rounded-2xl bg-neutral-700/30 border-neutral-700 gap-2">
          ...
          <div className="max-w-md">
            <p className="text-xl text-right text-neutral-400 font-[Quicksand] mb-4">
              Master JavaScript programming from basics to advanced DOM
              manipulation. Build interactive and dynamic websites with
              confidence and modern best practices.
            </p>
            ...
            <div className="flex flex-wrap justify-end gap-2 text-sm">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
                ...
                <span className="text-neutral-600">12 Lessons</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
                ...
                <span className="text-neutral-600">~6 Hours</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex items-stretch justify-center gap-6 ">
          ...
          >
            ...
            <p className="mb-6 leading-relaxed text-md text-neutral-400">
              Progress through each chapter to master JavaScript fundamentals
              and DOM manipulation. Click on any chapter to start learning, or
              use the menu to mark lessons as complete.
            </p>

            {/* CHAPTERS */}
            <div className="flex flex-col h-full gap-2">
              {chapters.map((chapter) => {
                const isCompleted = isLessonCompleted("javascript", chapter.id);
                return (
                  <div
                    onClick={() => navigate(`/lessons/js/${chapter.id}`)}
                    key={chapter.id}
                    ...
                    <div className="relative z-10 ml-auto">
                      {!isCompleted && (
                        <button
                          ...
                          className="flex items-center justify-center w-6 h-6 transition-colors rounded-full hover:bg-neutral-700"
                        >
          ...
          >
            ...
            <div className="p-6 border bg-neutral-800 rounded-xl border-neutral-700">
              <div className="flex items-center justify-center ">
                <span className="font-mono text-md text-neutral-400">
                  ...
                  {courseProgress?.totalLessons || 12} lessons
                </span>
              </div>
            ...
            {/* NEXT LESSON SECTION */}
            <div className="p-6 mt-6 border bg-gradient-to-r from-yellow-500/10 to-amber-500/10 rounded-xl border-yellow-500/30">
              <h3 className="mb-3 text-2xl font-semibold text-neutral-100">
                Continue Learning
              </h3>
              {(() => {
                const nextLesson = chapters.find(
                  (chapter) => !isLessonCompleted("javascript", chapter.id)
                );
                if (nextLesson) {
                  return (
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => navigate(`/lessons/js/${nextLesson.id}`)}
                        className="flex items-center justify-center w-10 h-10 text-sm font-medium text-white transition-colors duration-200 bg-yellow-500 rounded-full cursor-pointer hover:bg-yellow-600"
                      >
            ...
            <div className="p-6 mt-6 border bg-neutral-800/50 rounded-xl border-neutral-700">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                ...
                <div className="flex flex-col items-center p-3 border rounded-full border-yellow-500/40 bg-neutral-900/50 ">
                  <span className="text-xs text-neutral-500">Remaining</span>
                  <span className="font-mono text-xl text-yellow-400">
                    {(courseProgress?.totalLessons || 12) -
                      (courseProgress?.completedLessons.length || 0)}
                  </span>
                </div>
              </div>
