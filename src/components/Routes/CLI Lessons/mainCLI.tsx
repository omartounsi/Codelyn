...
export const MainCLI = () => {
  const { getCourseProgress, isLessonCompleted, markLessonComplete } =
    useProgress();
  const courseProgress = getCourseProgress("cli");
  const [openMenuId, setOpenMenuId] = useState<number | null>(null);

  const chapters = [
    { id: 1, title: "Introduction to Command Line" },
    { id: 2, title: "Navigation and Directory Structure" },
    { id: 3, title: "Working with Files" },
    { id: 4, title: "Terminal Management" },
  ...
  const handleMarkComplete = async (e: React.MouseEvent, chapterId: number) => {
    e.stopPropagation();
    await markLessonComplete("cli", chapterId);
    setOpenMenuId(null);
  };

  const handleOpenLesson = (chapterId: number) => {
    navigate(`/lessons/cli/${chapterId}`);
    setOpenMenuId(null);
  };
        ...
        <div className="flex items-center mt-10 mb-8 border justify-evenly w-[92%] mx-auto p-6 rounded-2xl bg-neutral-700/30 border-neutral-700 gap-2">
          ...
          <div className="max-w-md">
            <p className="text-xl text-right text-neutral-400 font-[Quicksand] mb-4">
              Master the command line and unlock the power of text-based
              computing. Learn essential navigation and file management skills
            ...
            <div className="flex flex-wrap justify-end gap-2 text-sm">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
                ...
                <span className="text-neutral-600">5 Lessons</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border rounded-full border-neutral-700/40 bg-neutral-800/50">
                ...
                <span className="text-neutral-600">~2 Hours</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex items-stretch justify-center gap-6 ">
          ...
          >
            ...
            <p className="mb-6 leading-relaxed text-md text-neutral-400">
              Progress through each chapter to master command line fundamentals.
            ...
            <div className="flex flex-col h-full gap-2">
              {chapters.map((chapter) => {
                const isCompleted = isLessonCompleted("cli", chapter.id);
                return (
                  <div
                    onClick={() => navigate(`/lessons/cli/${chapter.id}`)}
                    key={chapter.id}
          ...
          >
            ...
            <div className="p-6 border bg-neutral-800 rounded-xl border-neutral-700">
              <div className="flex items-center justify-center ">
                <span className="font-mono text-md text-neutral-400">
                  ...
                  {courseProgress?.totalLessons || 5} lessons
                </span>
              </div>
                ...
                <div className="relative w-40 h-40">
                  ...
                  >
                    ...
                    <circle
                      ...
                      r="46"
                      fill="none"
                      stroke="url(#progressGradientCLI)"
                      strokeWidth="8"
                      strokeDasharray="289"
            ...
            {/* NEXT LESSON SECTION */}
            <div className="p-6 mt-6 border bg-gradient-to-r from-green-500/10 to-cyan-400/10 rounded-xl border-green-500/30">
              <h3 className="mb-3 text-2xl font-semibold text-neutral-100">
                Continue Learning
              </h3>
              {(() => {
                const nextLesson = chapters.find(
                  (chapter) => !isLessonCompleted("cli", chapter.id)
                );
                if (nextLesson) {
                  return (
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() =>
                          navigate(`/lessons/cli/${nextLesson.id}`)
                        }
                        className="flex items-center justify-center w-10 h-10 text-sm font-medium text-white transition-colors duration-200 bg-green-500 rounded-full cursor-pointer hover:bg-green-600"
                      >
            ...
            <div className="p-6 mt-6 border bg-neutral-800/50 rounded-xl border-neutral-700">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                ...
                <div className="flex flex-col items-center p-3 border rounded-full border-green-500/40 bg-neutral-900/50 ">
                  <span className="text-xs text-neutral-500">Remaining</span>
                  <span className="font-mono text-xl text-green-400">
                    {(courseProgress?.totalLessons || 5) -
                      (courseProgress?.completedLessons.length || 0)}
                  </span>
                </div>
              </div>
